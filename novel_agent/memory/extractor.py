...
{{
  ...
  "new_state": {{
    ...
    "protagonist_tier": "主角当前境界（仅在正文明确突破时才提升）",
    "protagonist_location": "主角当前所在",
    "characters": [
      {{"name": "角色名", "power_tier": "境界", "status": "存活/重伤/死亡/失踪", "location": "所在", "note": "即时状态"}}
    ],
    "items": ["主角当前持有的关键物品"],
...
- 角色生死、境界等，必须以本章正文为准；正文没提到的角色，沿用旧状态。
...
def consolidate(
    ...
    *,
    ...
    data = gateway.complete_json(
        prompt, system=_SYSTEM, task="extract", max_tokens=4096
    )
